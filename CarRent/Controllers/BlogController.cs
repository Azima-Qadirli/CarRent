...
public class BlogController : Controller
{
    ...
    public async Task<IActionResult> Detail(int id)
    {
        ...
        ViewBag.Blogs = await _repository.GetAll().Include(x=>x.Author).OrderByDescending(x => x.CreateAt).Take(3).ToListAsync();
        var blog = await _repository.GetAll().Include(x=>x.Author).Where(x=>x.Id==id).Include(x=>x.BlogTags).ThenInclude(x=>x.Tag).FirstOrDefaultAsync();
        return View(blog);
    }
}